...
def calculate_user_metrics(orders_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for period_month in unique_months:
        ...
        month_end = (period_month + 1).to_timestamp() - pd.Timedelta(days=1)
        ...
        churn_users.append({
            'booking_month': period_month,
            'churn_warning_30_60d': churn_30_60d,
            'churn_warning_60_90d': churn_60_90d,
            'churned_90_120d': churn_90_120d,
        ...
        reactivated_users.append({
            'booking_month': period_month,
            'reactivated_users': reactivated_in_month
        })
...
def main_space():
    ...
    try:
        with connect_to_db(db_path) as conn:
            ...
            space_df['订单商品名'] = space_df['订单商品名'].str.replace('上海洛克外滩店-', '')
            space_df['订单商品名'] = space_df['订单商品名'].str.replace('the Box', '')
            ...
            member_results = analyze_member_value(space_df, db_path)
            ...
            save_analysis_to_excel(all_results, 'output_space.xlsx')
            print("Space analysis completed successfully")
